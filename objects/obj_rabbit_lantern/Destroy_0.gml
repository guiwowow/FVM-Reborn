...
if explode_timer <= 0{
	...
	var start_pos_x = x -global.grid_cell_size_x
	var start_pos_y = y - global.grid_cell_size_y
	for(var i = 0; i < 3;i++){
		for(var j= 0 ; j < 3;j++){
			...
			if grid_pos.row >= 0 && grid_pos.row < global.grid_rows && grid_pos.col >= 0 && grid_pos.col <= global.grid_cols{
				var inst = instance_create_depth(start_pos_x + j * global.grid_cell_size_x,start_pos_y + i * global.grid_cell_size_y,depth,obj_lantern_burn_effect)
				inst.damage = 25
				inst.max_time = 180
				if shape >= 1{
					inst.max_time = 300
	...
	with (obj_enemy_parent) {
		...
		if (abs(x - other.x) < _range && grid_row >= other.grid_row-1&&grid_row <= other.grid_row+1) {
	...
	if shape == 0 {
		effect_inst.sprite_index = spr_rabbit_lantern_explode
	}
	else if shape == 1{
		effect_inst.sprite_index = spr_rabbit_lantern_explode_1
	}
	else if shape == 2{
		effect_inst.sprite_index = spr_rabbit_lantern_explode_2
	}
}
event_inherited()
