// 三线射手发射子弹的代码
var middle_y = y + 50; // 中间行的y坐标
var row_height = 100; // 行高，根据你的游戏调整

// 创建三颗子弹，分别对应三行
var inst_middle = instance_create_depth(x + 60, middle_y, depth - 500, obj_waterpipe_bullet);
inst_middle.damage = atk;
inst_middle.move_speed = 8;
inst_middle.row = grid_row;
inst_middle.target_row = grid_row; // 目标行（中间行不变）

var inst_up = instance_create_depth(x + 60, middle_y, depth - 500, obj_waterpipe_bullet);
inst_up.damage = atk;
...
var inst_down = instance_create_depth(x + 60, middle_y, depth - 500, obj_waterpipe_bullet);
